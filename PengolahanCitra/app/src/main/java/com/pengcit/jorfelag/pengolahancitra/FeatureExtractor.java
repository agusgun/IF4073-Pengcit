...
public class FeatureExtractor {
    ...
    // Get Frequency of Chaincode
    // | 8 | 1 | 2 |
    // | 7 | 0 | 3 |
    // | 6 | 5 | 4 |
    public ArrayList<Integer> getChainCodeFrequency() {
        boolean[][] used = new boolean[imageMatrix.length][imageMatrix[0].length];
        for (int i = 0; i < imageMatrix.length; i++) {
            for (int j = 0; j < imageMatrix[0].length; j++) {
                used[i][j] = false;
        ...
        Point firstPixel = getFirstPixel();

        // Find Nearest Endpoint from FirstPixel
        Point startPixel = new Point();
        int minDistance = 99999999;
        Iterator<Point> itEndpoint = endPoints.iterator();
        if (itEndpoint.hasNext()) {
            Point curr = itEndpoint.next();
            int distance = Math.abs(curr.x - firstPixel.x) + Math.abs(curr.y - firstPixel.y);
            if (distance < minDistance) {
                startPixel.set(curr.x, curr.y);
    ...
    public ArrayList<Integer> getChainCodeFrequencyHelper(Point startPoint, boolean[][] used) {
        ArrayList<Integer> ans = new ArrayList<>();
        for (int i = 0; i < 9; i++) {
            ans.add(i, 0);
        }
